...
@com.qualcomm.robotcore.eventloop.opmode.TeleOp(name="Final_competition_Code_2024_v4", group="Linear Opmode")
public class Final_competition_Code_2024_v4 extends LinearOpMode {
...
int linearSlidePosition;
 // Set servo to mid position
    ...
    ServoSpeed2 = 0.1;
      ...
      ServoPosition2 = Math.min(Math.max(ServoPosition2, 0.00001), 0.5);
      Claw.setPosition(ServoPosition2);

         if (gamepad2.right_stick_button && xTimer.milliseconds() >= 200) { 
               ArmRunning = !ArmRunning; 
              xTimer.reset();
               }
  ...
  if (gamepad2.left_stick_y > 0.1) {
    ArmLeft.setPower(0.4*gamepad2.left_stick_y); // Move up
    ArmRight.setPower(0.4*gamepad2.left_stick_y); // Move up
} else if (gamepad2.left_stick_y < -0.1)  { 
    ArmRight.setPower(0.7*gamepad2.left_stick_y); 
    ArmLeft.setPower(0.7*gamepad2.left_stick_y);
}else if (ArmRunning){
      ArmRight.setPower(1);
      ArmLeft.setPower(1);
  }
